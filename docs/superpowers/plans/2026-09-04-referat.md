...
| `Sources/Schous/SummaryView.swift` (ny) | `SummarySheet` (mal/modell/språk/kontekst) og `SummaryPanel` (teksten) |
...
| `Sources/Schous/SpeakerEditorView.swift` | knappen, arket, panelet, status |
...
enum Ollama {
    ...
    /// kjøres når Innstillinger og referat-arket åpnes, ikke i bakgrunnen.
...
```swift
    ...
    func refreshModels() async {
        ...
        // Første gang: velg noe, så arket ikke står med tom modell.
...
### Task 5: Editoren — ark, panel, status

**Files:**
- Create: `Sources/Schous/SummaryView.swift`
- Modify: `Sources/Schous/SpeakerEditorView.swift` (`@State`-er ~linje 10, `body` ~linje 38, `transcript` ~linje 65, `loadMapping` ~linje 190)

**Interfaces:**
- Consumes: `Summarizer`, `Templates`, `Summary`, `SummaryLanguage`, `Ollama.models`, `AppSettings.shared.{ollamaBaseURL, summaryModel, summaryLanguage, summaryPrompt}`, `transcriptText`, `SpeakerEditorView.resolved`, `job.jobDir`, `job.base`, `job.segments`.
- Produces: `struct SummarySheet: View` med `init(jobDir: URL?, onStart: @escaping (_ template: URL, _ model: String, _ language: SummaryLanguage, _ context: String) -> Void)`; `struct SummaryPanel: View` med `init(summarizer: Summarizer)`.
...
struct SummarySheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Lag referat").font(.headline)
            if templates.isEmpty {
                Text("Ingen maler i malmappa.").foregroundStyle(.orange)
            ...
            Text("Kontekst (valgfritt): hvem var med, hva gjaldt møtet")
                .font(.caption).foregroundStyle(.secondary)
            TextEditor(text: $context).frame(height: 80)
            HStack {
                Spacer()
...
/// Teksten mens den strømmer inn, og etterpå.
...
```swift
            VSplitView {
                transcript
                if !summarizer.text.isEmpty || summarizer.state == .running {
                    SummaryPanel(summarizer: summarizer)
                }
            }
```

I `.toolbar`, i `ToolbarItem(placement: .status)`, erstatt innholdet:

```swift
                HStack(spacing: 8) {
                    if summarizer.state == .running, let t0 = summarizer.started {
                        Text("Referat … ") + Text(t0, style: .timer)
                    } else if case .done(let url) = summarizer.state {
                        Text("Referat lagret som \(url.lastPathComponent)").font(.callout)
                        Button("Vis i Finder") { NSWorkspace.shared.activateFileViewerSelecting([url]) }
                            .buttonStyle(.link)
                    } else if case .failed(let msg) = summarizer.state {
...
I `ToolbarItemGroup(placement: .primaryAction)`, før `Menu("Lagre")`:

```swift
                if summarizer.state == .running {
                    Button("Stopp") { summarizer.cancel() }
                } else {
                    Button("Lag referat") { showSummarySheet = true }
                }
```

Etter `.onAppear(perform: loadMapping)`:

```swift
        .sheet(isPresented: $showSummarySheet) {
            SummarySheet(jobDir: job.jobDir, onStart: startSummary)
        }
```
...
```swift
    ...
    /// Navnene tas fra `resolved` nå. Endrer du et navn etterpå, er det en ny kjøring.
...
Deretter (Task 6 er ikke gjort ennå, så bruk en fersk transkribering av en *kort* fil, eller vent til Task 6 og kjør denne sjekken der). Hvis du kjører nå: `pkill -x Schous; open Schous.app --args --input <kort lydfil>`, Start, vent til editoren. Trykk «Lag referat»: arket viser maler, modell forhåndsvalgt, språk. Start. Expected: knappen blir «Stopp», status «Referat … 0:03», teksten vokser i panelet under transkripsjonen. Etterpå: «Referat lagret som <base>.stand-up.md» og «Vis i Finder» markerer fila. `ls <output-mappa>/*.md` og `ls <jobbmappa>/summary.*.md <jobbmappa>/context.txt`.

Trykk «Stopp» midt i en kjøring: knappen tilbake til «Lag referat», ingen ny fil, status tom.

`git status` — ingen media.

- [ ] **Step 4: Commit**

```bash
git add Sources/Schous/SummaryView.swift Sources/Schous/SpeakerEditorView.swift
git commit -m "Editoren: «Lag referat» med mal, modell, språk og kontekst; teksten strømmer inn under transkripsjonen"
```
...
Once the speakers are named, click **Lag referat**. Pick a template, a model
and a language; optionally add context (who was in the room, what the meeting
was about). The summary streams into the window and is written next to the
transcript as `<file>.<template>.md`.
...
```bash
gh issue comment 34 --body "Referat-PR-en legger inn et fritekstfelt «Kontekst» i referat-arket, lagret som \`context.txt\` i jobbmappa. Det er feltet en deltakerliste ville trengt; matchingen mot taler-ID-er er fortsatt ikke bygget, og kravet fra #32 (forslag med belegg, aldri skrevet rett inn) står."
```
...
- Editoren (knapp, ark, panel, status med tid, Vis i Finder, Stopp, navn fra `resolved` ved start, forrige referat vises): Task 5. ✓
